package com.x2xcoin.wallet.data;
...
import com.x2xcoin.wallet.core.tx.TransactionSigner;
import com.x2xcoin.wallet.core.wallet.Amount;
import com.x2xcoin.wallet.core.wallet.WalletAccount;
import com.x2xcoin.wallet.core.wallet.WalletStore;
...
public final class WalletRepository {
    private static final String PREFS = "x2x_wallet_secure";
    ...
    private final X2xApiClient apiClient;
    private final ExecutorService executor = Executors.newSingleThreadExecutor();
    private WalletStore walletStore = new WalletStore();
    private char[] sessionPassword;

    public WalletRepository(Context context, X2xApiClient apiClient) {
    ...
    public X2xApiClient api() {
        return apiClient;
    }

    public X2xApiClient.BalanceResponse refreshBalanceResponse(String address, boolean invalidateCache) throws Exception {
        if (invalidateCache) {
            try {
    ...
    public String send(String destination, String amountText) throws Exception {
        ...
        List<X2xApiClient.Utxo> utxos = apiClient.getUtxos(active.address());
        List<TransactionSigner.SpendableUtxo> spendable = new ArrayList<>();
        for (X2xApiClient.Utxo utxo : utxos) {
